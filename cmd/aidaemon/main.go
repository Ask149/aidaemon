...
func doLoginTeams(cfg *config.Config) {
	...
	tok, err := auth.RunEntraDeviceFlow(cfg.TeamsClientID, cfg.TeamsTenantID, cfg.TeamsScopes)
	if err != nil {
		log.Fatalf("teams login failed: %v", err)
	}
	etm, err := auth.NewEntraTokenManager(cfg.TeamsClientID, cfg.TeamsTenantID, cfg.TeamsScopes)
	if err != nil {
...
func run() error {
	...
	// 5h. Register search_history tool (needs store).
	registry.Register(&builtin.SearchHistoryTool{
		Store: st,
	})
...
func setupTools(cfg *config.Config) *tools.Registry {
	...
	allowedFilePaths := []string{
		filepath.Join(home, ".config", "aidaemon"),
	...
	// Calendar, email, and goals tools.
	registry.Register(&builtin.CalendarTool{})
	registry.Register(&builtin.EmailTool{})

	configDir, _ := os.UserConfigDir()
	goalsPath := filepath.Join(configDir, "aidaemon", "heartbeat", "goals.jsonl")
	registry.Register(builtin.NewGoalsTool(goalsPath))
