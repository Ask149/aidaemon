...
func New(cfg Config) (*Bot, error) {
	...
	b.RegisterHandler(bot.HandlerTypeMessageText, "/task", bot.MatchTypePrefix, tb.handleTaskQueue)
...
// handleTaskQueue appends a user task to the heartbeat task queue file.
// The heartbeat system (opencode cron) picks up queued tasks on its next run.
func (tb *Bot) handleTaskQueue(ctx context.Context, b *bot.Bot, update *models.Update) {
	if !tb.isAuthorized(update.Message.Chat.ID) {
		return
	}

	text := update.Message.Text
	taskText := strings.TrimSpace(strings.TrimPrefix(text, "/task"))
	if taskText == "" {
		tb.sendText(ctx, update.Message.Chat.ID, "Usage: /task <your request>\nExample: /task check calendar for tomorrow")
		return
	}

	queueFile := filepath.Join(os.Getenv("HOME"), ".config", "opencode", "heartbeat", "state", "task-queue.txt")

	// Ensure parent directory exists.
	if err := os.MkdirAll(filepath.Dir(queueFile), 0755); err != nil {
		log.Printf("[telegram] /task: mkdir failed: %v", err)
		tb.sendText(ctx, update.Message.Chat.ID, "❌ Failed to queue task")
		return
	}

	entry := fmt.Sprintf("%s | %s\n", time.Now().Format("2006-01-02 15:04:05"), taskText)

	f, err := os.OpenFile(queueFile, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		log.Printf("[telegram] /task: open queue file: %v", err)
		tb.sendText(ctx, update.Message.Chat.ID, "❌ Failed to queue task")
		return
	}
	defer f.Close()
	...
	if _, err := f.WriteString(entry); err != nil {
		log.Printf("[telegram] /task: write to queue: %v", err)
		tb.sendText(ctx, update.Message.Chat.ID, "❌ Failed to write task")
		return
	}

	tb.sendText(ctx, update.Message.Chat.ID, "✅ Task queued for next heartbeat (runs every 30 min)")
}
...
func (tb *Bot) maybeAutoScreenshot(ctx context.Context, chatID int64, toolName string) {
	...
	for attempt := 1; attempt <= 2; attempt++ {
		result, err := mcpTool.Client().CallTool(ctx, "browser_take_screenshot", map[string]interface{}{})
		if err != nil {
