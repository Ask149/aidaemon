...
type Config struct {
	...
	TeamsScopes       string `json:"teams_scopes"`        // override Graph API scopes (default: Chat.Read ChatMessage.Send User.Read offline_access)
	TeamsWebhookURL   string `json:"teams_webhook_url"`   // if set, Send() uses Incoming Webhook instead of Graph API
}
