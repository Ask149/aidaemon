...
type Conversation interface {
	...
	// AddFact upserts a fact (insert or update by key).
	AddFact(key, value, category string) error

	// GetFacts returns facts filtered by optional query (substring) and category.
	GetFacts(query, category string) ([]Fact, error)
...
func (s *SQLiteStore) migrate() error {
	...
	// Migrate FTS (full-text search) table.
	if err := s.migrateFTS(); err != nil {
		return err
	}

	// Migrate facts table.
	if err := s.migrateFacts(); err != nil {
		return err
	}
