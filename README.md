...
> **Looking for a desktop companion?** [Friday](https://github.com/Ask149/friday) is a macOS menu-bar app with an animated face, voice I/O, and personality — powered by aidaemon.
...
- **Tool execution** — read/write files, run shell commands, search the web, manage calendar, email, and goals
...
- Azure AD app registration with delegated permissions: `Chat.Read`, `ChatMessage.Send`, `User.Read`, `offline_access`
...
5. After creation, go to **API permissions** > **Add a permission** > **Microsoft Graph** > **Delegated permissions**:
   ...
   - `User.Read`
   - `offline_access`
6. Go to **Authentication** → **Advanced settings** → set **Allow public client flows** to **Yes** (required for device code flow)
7. Note down: **Application (client) ID** and **Directory (tenant) ID**
...
```jsonc
{
  ...
  "teams_scopes": "",                   // optional, override Graph API scopes
  "teams_webhook_url": ""               // optional, send via Incoming Webhook
}
```

**Config fields:**
- `teams_scopes` — Override Graph API scopes. Default: `"Chat.Read ChatMessage.Send User.Read offline_access"`. Set to `"Chat.ReadWrite User.Read offline_access"` for backward compatibility.
- `teams_webhook_url` — If set, messages are sent via Incoming Webhook instead of Graph API. Set up: Teams channel → Connectors → Incoming Webhook → copy URL.
...
AIDaemon includes a web UI at `http://localhost:8420` (configurable via `port` in config):
...
| `search_history` | Search past conversation history across sessions |
| `write_workspace` | Write structured output to workspace files |
| `calendar` | Check Google Calendar events (today, upcoming, search) |
| `email` | Read and search Gmail messages |
| `goals` | Track and manage personal goals and progress |
...
```jsonc
{
  ...
  "teams_scopes": "string",           // Override Graph API scopes (default: "Chat.Read ChatMessage.Send User.Read offline_access")
  "teams_webhook_url": "string",      // Send via Incoming Webhook instead of Graph API
...
See [TOOLS.md](TOOLS.md) for how to build custom tools.  
See [MCP.md](MCP.md) for MCP server integration.
...
scripts/
  ...
  com.aidaemon.watchdog.plist.template macOS launchd agent template
  com.friday.watchdog.plist.template   Companion app watchdog template
...
internal/
  ...
  tools/
    builtin/             Built-in tools (11 tools)
...
| `scripts/com.aidaemon.watchdog.plist.template` | macOS launchd agent template (replace `__HOME__` and `__AIDAEMON_DIR__` with your paths) |
