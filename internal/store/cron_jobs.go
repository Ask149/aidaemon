...
func (s *SQLiteStore) UpdateCronJob(job CronJob) error {
	...
	_, err := s.db.Exec(`
		UPDATE cron_jobs
		SET label = ?, cron_expr = ?, mode = ?, payload = ?, channel_type = ?, channel_meta = ?, enabled = ?, last_run_at = ?, next_run_at = ?
		WHERE id = ?
	`,
		job.Label, job.CronExpr, job.Mode, job.Payload, job.ChannelType, job.ChannelMeta, enabled,
