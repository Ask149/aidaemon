...
// TestSendWebhook verifies that Send() POSTs an Adaptive Card to the webhook URL
// when WebhookURL is configured, instead of using the Graph API.
func TestSendWebhook(t *testing.T) {
	...
	ch := &Channel{
		...
		graphBaseURL: "http://should-not-be-called",
		httpClient:   server.Client(),
	}

	err := ch.Send(context.Background(), "teams:test-chat-id", "Hello via webhook")
	if err != nil {
		t.Fatalf("Send returned error: %v", err)
	}
	...
	// Verify it's an Adaptive Card message envelope.
	msgType, _ := gotBody["type"].(string)
	if msgType != "message" {
		t.Errorf("type = %q, want %q", msgType, "message")
	}
	...
	if att["contentType"] != "application/vnd.microsoft.card.adaptive" {
		t.Errorf("contentType = %q, want adaptive card", att["contentType"])
	}

	// Verify the text is in the card body.
	content := att["content"].(map[string]interface{})
	body := content["body"].([]interface{})
	textBlock := body[0].(map[string]interface{})
	if textBlock["text"] != "Hello via webhook" {
		t.Errorf("text = %q, want %q", textBlock["text"], "Hello via webhook")
	}
}

// TestSendWebhook_Error verifies that webhook errors are propagated.
func TestSendWebhook_Error(t *testing.T) {
	...
	err := ch.Send(context.Background(), "teams:test-chat-id", "Hello")
	if err == nil {
		t.Fatal("Send should return error on webhook failure")
	}
...
// TestSendGraphAPI_StillWorks verifies that without WebhookURL, Send() still uses Graph API.
func TestSendGraphAPI_StillWorks(t *testing.T) {
	var gotPath string
	...
	ch := &Channel{
		cfg: Config{
			ChatID: "test-chat-id",
			// WebhookURL intentionally empty — should use Graph API
		},
		graphBaseURL: server.URL + "/v1.0",
		httpClient:   server.Client(),
	}

	err := ch.Send(context.Background(), "teams:test-chat-id", "Hello via Graph")
	if err != nil {
		t.Fatalf("Send returned error: %v", err)
	}

	expectedPath := "/v1.0/me/chats/test-chat-id/messages"
	if gotPath != expectedPath {
