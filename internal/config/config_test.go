...
func TestTeamsScopes_JSON(t *testing.T) {
	jsonStr := `{"chat_model":"test","teams_scopes":"Chat.ReadWrite User.Read offline_access"}`
	cfg := DefaultConfig()
	if err := json.Unmarshal([]byte(jsonStr), &cfg); err != nil {
		t.Fatalf("unmarshal: %v", err)
	}
	if cfg.TeamsScopes != "Chat.ReadWrite User.Read offline_access" {
		t.Errorf("TeamsScopes = %q, want custom value", cfg.TeamsScopes)
	}
}

func TestTeamsWebhookURL_JSON(t *testing.T) {
	jsonStr := `{"chat_model":"test","teams_webhook_url":"https://example.webhook.office.com/webhookb2/..."}`
	cfg := DefaultConfig()
	if err := json.Unmarshal([]byte(jsonStr), &cfg); err != nil {
		t.Fatalf("unmarshal: %v", err)
	}
	if cfg.TeamsWebhookURL != "https://example.webhook.office.com/webhookb2/..." {
		t.Errorf("TeamsWebhookURL = %q, want webhook URL", cfg.TeamsWebhookURL)
	}
}
